...
function AdminDashboard() {
  ...
  const fetchCourses = async () => {
    try {
      ...
      alert('Failed to fetch courses: ' + (err.response?.data?.error || 'Server error'));
    }
  };
  ...
  const handleGenerate = async () => {
    ...
    try {
      await axios.post('http://localhost:3001/generate-timetable', { semester: parseInt(semester) }, {
        headers: { Authorization: `Bearer ${token}` },
      });
      alert(`Timetable generation started for Semester ${semester}. Refresh to see results.`);
      setTimeout(fetchTimetable, 2000);
    } catch (err) {
      alert('Timetable generation failed: ' + (err.response?.data?.error || 'Server error'));
    }
  };
  ...
  const handleAddCourse = async (e) => {
    ...
    try {
      ...
      alert('Failed to add course: ' + (err.response?.data?.error || 'Server error'));
    }
  };
  ...
  return (
    <div className="admin-dashboard-container">
      ...
      <div className="controls-section">
        ...
        <button onClick={handleGenerate} className="generate-btn">Generate Timetable</button>
      </div>
      ...
      <div className="download-buttons">
        ...
        <button onClick={() => handleDownload('excel')} className="download-btn excel-btn">
          Download Excel
        </button>
